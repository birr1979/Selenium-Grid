...
public class costcoCartTestCase {
	/**
	 * This class is a test case for Costco.com webPage, to check for delivery ZipCode Update, 
	 * Search for Products, and Adding/ removing Items to/form the cart
	 * All WebElement Locators are found in this class as a static variable
	 * Also a method is included to capture all the products description in the search result 
	 *   
	 */
	...
	public static void main(String[] args) throws InterruptedException {
		...
		//				driver.get(URL);
		//				System.out.println("Info: Navigated to Costco Home Page titled: "+driver.getTitle());
		//		
		...
		/**
		 * SInce the Costco home page is not responding due to the Web site compatibility issue
		 * I have bypassed the link to the beverage section 
		 * which is the successive code of the above commented code. 
		 */
		...
		//		driver.findElement(By.xpath(ChangeDeliveryZipCode_XPATH)).click();  //>>>>>Optional some times needed 
		...
		System.out.println("Cart is Updated and now has: "+driver.findElement(By.xpath(cartItemAmount_XPATH)).getText()+" item in the cart.");
		...
		System.out.println("Serach results shown and Header appears saying: "+driver.findElement(By.xpath(SearchResult_XPATH)).getText()); //verify??

		//Displaying all the name of the Product in the first page, search results
		wait= new WebDriverWait(driver,1000); //to wait for elements that are being loaded to the DOM after some action is performed.
		...
		for(int i=0;i<(ProductsName.size());i++) {
			...
			//Checking if ‘Syrup’ is a part of at least 1 product’s name that is displayed above
		...
		//Adding the 2ND Item to the cart (the 1st item from the Breakfast search result)
		...
		System.out.println("Cart is Updated and now has: "+driver.findElement(By.xpath(cartItemAmount_XPATH)).getText()+" in the cart.");
		...
		//Increase the quantity of the 1st product in the cart using Java Script Executer
		...
		System.out.println("The first Item in the cart is increased by quantity of 1 and Cart Total is =$ "+CartTotal1$);
		...
		if(CartTotal2$>0 && CartTotal2$<CartTotal1$) {
			System.out.println("Cart total was: $"+CartTotal1$+" and now Cart Total is: $"+CartTotal2$+" which is less: $"+(CartTotal1$-CartTotal2$)+" by Item 2 amount.");
		}else {
